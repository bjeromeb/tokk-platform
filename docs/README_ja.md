...
- **--version**: デプロイする Tokk AI Platform のバージョン。 (デフォルト: 開発中の最新バージョン)
...
> [!TIP] > `Frontend URL` が正しく表示されない場合や、Tokk AI Platform が正常に動作しない場合は、最新バージョンの不具合である可能性がありますので、パラメータに `--version "v1.2.6"` を追加して再度デプロイを試してみてください。
